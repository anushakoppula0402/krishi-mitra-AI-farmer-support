const machineryData = [
  {
    ...
    machinery: [
      ...
      },
      {
        ...
        image: 'https://images.unsplash.com/photo-1574323347407-f5e1ad6d020b?w=500&h=300&fit=crop&crop=center',
        cost: 45000,
        subsidy: {
          available: true,
          details: {
      ...
      },
      {
        ...
        image: 'https://images.unsplash.com/photo-1625246333195-78d9c38ad449?w=500&h=300&fit=crop&crop=center',
        cost: 75000,
        subsidy: {
          available: true,
          details: {
    ...
    machinery: [
      {
        ...
        image: 'https://images.unsplash.com/photo-1595863045629-e10b1e0c50b8?w=500&h=300&fit=crop&crop=center',
        cost: 85000,
        subsidy: {
          available: true,
          details: {
    ...
    machinery: [
      {
        ...
        description: {
          ...
          te: 'చెట్ల వేరులకు నేరుగా నీళ్ళని పంపించే నీళ్ళు-కార్యక்షమమైన రీతి వ్యవస్థ.',
        },
...
// Analyze image usage
const machines = machineryData.flatMap(cat => cat.machinery);
const imageMap = {};

machines.forEach(machine => {
  const name = machine.name.en;
  const image = machine.image;
  
  if (!imageMap[image]) {
    imageMap[image] = [];
  }
  imageMap[image].push(name);
});
...
console.log('\n=== DUPLICATE IMAGES ===');
Object.entries(imageMap).forEach(([image, names]) => {
  if (names.length > 1) {
    console.log('Image:', image);
    console.log('Used by machines:', names.join(', '));
    console.log('---');
  }
});

console.log('\n=== IMAGE TO MACHINE MAPPING ===');
Object.entries(imageMap).forEach(([image, names]) => {
  console.log('Image:', image);
